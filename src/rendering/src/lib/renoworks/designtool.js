...
import { decodeHtml, encodeUrl } from 'lib/utils/string-utils';
import { RenoworksClient, RenoworksKeyLocation, RenoworksKeyUsage, RenoworksProductSide } from './renoworks';
...
export class ViewModelBuilderBase {
  constructor(product, apiConfig) {
    ...
    this._renoworksClient = new RenoworksClient(product.renoworksKey, apiConfig.host, apiConfig.prefix);
  ...
  buildSummary() {
    var summary = new SummaryViewModel(this.product, null, this._viewModel.interiorImage, this._viewModel.exteriorImage, this._viewModel.selectedOptions, DesignToolStepName.Summary);
    return summary;
  }
  ...
  _updateProductSettingValuesFromRenoworksResult(productSettingValues, result, productSettings) {
    ...
    for (let returnedSetting of returnedSettings)
    {
      ...
      var productSetting = productSettings.singleOrDefault(productSetting => productSetting.renoworksKey.name === settingName);
        ...
        if (productSetting.childKeys !== null)
        {
          ...
          if (valueParts.length > 1)
          {
            ...
            for (let value of valueParts)
            {
              ...
              var childKey = productSetting.childKeys.singleOrDefault(_ => _.renoworksKey.name === childName);
              if (childKey !== null)
              {
...
export class SummaryViewModel extends AttributeViewModelBase {
  ...
  constructor(product, relatedProduct, interiorImage, exteriorImage, selectedOptions, renoworksKeyName, productSide) {
