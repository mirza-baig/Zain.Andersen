...
import { PageVariantMiddleware } from 'lib/personalization';
 ...
 */
class PageVariantPlugin implements MiddlewarePlugin {
  private pageVariantMiddleware: PageVariantMiddleware;

  // Using 20 to leave room for things like rewrites and redirects to occur first
  order = 20;

  constructor() {
    this.pageVariantMiddleware = new PageVariantMiddleware({
  ...
  async exec(req: NextRequest, res?: NextResponse): Promise<NextResponse> {
    return this.pageVariantMiddleware.getHandler()(req, res);
  }
}
