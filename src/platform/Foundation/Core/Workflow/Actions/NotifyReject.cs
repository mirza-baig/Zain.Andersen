...
namespace Platform.Foundation.Core.Workflow.Actions
{
    public class NotifyReject
    {
        ...
        public void Process(WorkflowPipelineArgs args)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(_submittingUser.Profile.Email))
                {
                    _emailService.SendEmail(
                        ...
                        subject: $"Affiliates – {contentItem.TemplateName} Reviewed",
                        body: Body(args),
                        isHtml: true);
                    Sitecore.Diagnostics.Log.Error(
                        "Email sent for rejection", this);
                }
                else
                {
                    Sitecore.Diagnostics.Log.Error(
            ...
            {
                Sitecore.Diagnostics.Log.Error("NotifyRect Process" + ex.Message, ex, this);
            }
        }
        ...
        private string Body(WorkflowPipelineArgs args)
        {
            ...
            body.AppendLine("<p>Your content has been <strong>declined</strong>.</p>");
