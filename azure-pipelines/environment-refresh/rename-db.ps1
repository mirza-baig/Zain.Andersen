...
Write-Host "Renaming current databases..."
...
foreach ($database in $databases) {
  $databaseOldName = $prefix + $database
  $databaseNewName = $prefix + $database + "_tmp"

  $progress = (($current - 1) / $count * 100.0)

	Write-Host "Renaming '$databaseOldName' to '$databaseNewName' on $server"
  Write-Host "##vso[task.setprogress value=$progress;]Renaming $current of $count - '$databaseOldName' -> '$databaseNewName'"

  $command = "ALTER DATABASE [$databaseOldName] MODIFY NAME = [$databaseNewName]"
